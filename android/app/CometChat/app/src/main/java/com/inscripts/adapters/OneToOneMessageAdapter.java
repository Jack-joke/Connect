...
import com.inscripts.interfaces.ImageUploadCallback;
...
public class OneToOneMessageAdapter extends ArrayAdapter<OneOnOneMessage> implements StickyListHeadersAdapter,ImageUploadCallback{

    private static final int TYPES_COUNT = 2;
    private static final int TYPE_LEFT = 0;
    private static final int TYPE_RIGHT = 1;
    private static final java.lang.String TAG = OneToOneMessageAdapter.class.getSimpleName();
    ...
    private ProgressWheel imageuploadProgressWeel;

    public OneToOneMessageAdapter(Activity context, List<OneOnOneMessage> messageList) {
        super(context, R.layout.custom_chat_message_one_on_one_left, messageList);
    ...
    public void onProgress(final float progress) {
        if(imageuploadProgressWeel!= null){

            context.runOnUiThread(new Runnable() {
                @Override
                public void run() {

                    Logger.error(TAG,"Progress value on ui thread = "+progress);
                    if(progress<100){
                        imageuploadProgressWeel.setBarColor(Color.WHITE);
                        imageuploadProgressWeel.setVisibility(View.VISIBLE);

                        imageuploadProgressWeel.setProgress(progress);
                    }else{
                        imageuploadProgressWeel.setVisibility(View.GONE);
                    }
                }
            });
    ...
    private static class Holder {
        ...
        public ImageView imageHolder, audioPlayButton, videoThumb, videoMessageButton, messageTick,customImageHolder, imagePendingMessage;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (vi == null) {
            ...
            if (getItemViewType(position) == TYPE_RIGHT) {
                vi = LayoutInflater.from(getContext()).inflate(R.layout.custom_chat_message_one_on_one_right, parent,
                        false);
                ...
                vi = LayoutInflater.from(getContext()).inflate(R.layout.custom_chat_message_one_on_one_left, parent,
                        false);
        ...
        switch (message.type) {
            ...
            case CometChatKeys.MessageTypeKeys.IMAGE_DOWNLOADED:
                ...
                holder.message.setVisibility(View.GONE);
                holder.wheel.setVisibility(View.GONE);
                imageuploadProgressWeel = holder.wheel;
                holder.imageHolder.setVisibility(View.GONE);
                holder.customImageHolder.setVisibility(View.VISIBLE);
                holder.tvOnlySmiley.setVisibility(View.GONE);
                ...
                if(getItemViewType(position) == TYPE_RIGHT){
                    holder.avatar.setVisibility(View.GONE);
                    Logger.error(TAG,"Image Remote id = "+message.remoteId);
                    if(message.remoteId == 0L){
                        holder.imagePendingMessage.setVisibility(View.VISIBLE);
                        holder.messageTick.setVisibility(View.GONE);
                        if(message.remoteId == 0L){
            ...
            case CometChatKeys.MessageTypeKeys.VIDEO_NEW:
                ...
                final ProgressWheel wheel = holder.wheel;
			 ...
			 * holder.videoThumb.setVisibility(View.VISIBLE);
			 */
                ProgressWheel wheel3 = holder.wheel;
            ...
            case CometChatKeys.MessageTypeKeys.STICKERS:
                ...
                Logger.error(TAG,"Sticker Message value = "+message.message);


                    if(message.message.contains("CC^CONTROL_")){
                        holder.message.setText(Stickers.handleSticker(message.message));
                    }else{
                        holder.message.setText(Stickers.getSpannableStickerString(message.message));
                    }
