...
public class VideoChat {
    ...
    public static String getAudioConferenceRoomName(String message) {
        int initialIndex;
        initialIndex = message.indexOf(CometChatKeys.AudiochatKeys.AUDIO_CrREQUEST_KEY);
        message = message.substring(initialIndex, message.length());
        String parts[] = message.split("\'");
        String roomname = parts[1];
        return roomname;
    ...
    public static void startAudioConference(String roomName, Activity sourceActivity) {
        Intent intent = new Intent(sourceActivity, VideoChatActivity.class);
        intent.putExtra(StaticMembers.INTENT_ROOM_NAME, roomName);
