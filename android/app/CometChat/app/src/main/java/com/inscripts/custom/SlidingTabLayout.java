 ...
 * {@link #setCustomTabView(int, int, int)}, providing the layout ID of your custom
 * layout.
 */
public class SlidingTabLayout extends HorizontalScrollView {
     ...
     * view is not set via {@link #setCustomTabView(int, int, int)}.
     */
    @SuppressLint("NewApi")
    ...
    private void populateTabStrip() {
        ...
        if (null != theme && null != theme.getActionbarColor()) {
            backgroundColor = Color.parseColor(theme.getActionbarColor());
        } else if (null != css) {
            backgroundColor = Color.parseColor(css.getTabTitleBackground());
        } else {
            backgroundColor = Color.parseColor(StaticMembers.COMETCHAT_DARK_GREEN);
        }
        int i, len = adapter.getCount();
        for (i = 0; i < len; i++) {
            View tabView = null;
            ...
            if (mTabViewLayoutId != 0) {
                ...
                GradientDrawable drawable = (GradientDrawable) tabTitleView.getBackground();
            ...
            String data[] = adapter.getPageTitle(i).toString().split("#");
            String imageName = data[0], tag = data[1];
            tabView.setTag(tag);

            tabImageView.setImageResource(Integer.parseInt(imageName));
	...
	/*My custom code
     *  Helpful to update badge count
	 * */

    public void updateBadgeCount(String tag, String count) {
        if (mTabStrip != null) {
            try {
                int i, len = mTabStrip.getChildCount();
                for (i = 0; i < len; i++) {
                    View view = mTabStrip.getChildAt(i);
                    if (tag.equals(view.getTag())) {
                        TextView tv = (TextView) view.findViewById(mTabViewTextViewId);
