...
//    private String set = "Set";
    private LinearLayout viewBots,viewChatSetting,viewNotificationSetting,viewLanguage,viewBlockedUser,viewGames,viewShareApp,viewInviteContact,viewEditGuestname;
    private String set = "Set";
    ...
    private TextView tvLanguageSubtitle,tvBlockUserSubtitle,tvNotificationSettingSub,tvBots;
    private ImageView imgBots,imgChatSetting,imgNotificationSetting,imgSettinglanguage,imgBlockUser,imgSettingGames,imgShareApp,imginvitePhoneContacts,imgEditUserName;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        viewProfile = (LinearLayout) findViewById(R.id.ll_view_profile);
        viewChatSetting = (LinearLayout) findViewById(R.id.ll_chat_setting);
        viewNotificationSetting = (LinearLayout) findViewById(R.id.ll_notification_setting);
    ...
    protected void onResume() {
        ...
        assert tvNotificationSettingSub != null;
        setNotificationSubTitle();
        ...
        if(notification != null && !notification.equals("0")){
            if(sound != null && vibarate != null){
                if(sound.equals("1") && vibarate.equals("1")){
                    tvNotificationSettingSub.setText(mobileLangs.get98()+" "+core.get36()+mobileLangs.get99());
                }else if(vibarate.equals("1")) {
                    tvNotificationSettingSub.setText(mobileLangs.get99());
                }else if(sound.equals("1")){
                    tvNotificationSettingSub.setText(mobileLangs.get98());
                }
            }else {
                tvNotificationSettingSub.setText(mobileLangs.get98()+" "+core.get36()+mobileLangs.get99());
            }
    ...
    private void setThemeColor(){
        ...
        ivViewProfile.getBackground().setColorFilter(Color.parseColor(primaryColor), PorterDuff.Mode.SRC_ATOP);
        ...
        imgNotificationSetting.getBackground().setColorFilter(Color.parseColor(primaryColor), PorterDuff.Mode.SRC_ATOP);
    ...
    private void setUpSetting(){
        ...
        viewProfile.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent profileIntent = new Intent(SettingActivity.this, ViewProfileActivity.class);
        ...
        if (null != JsonPhp.getInstance().getLang() && null != JsonPhp.getInstance().getLang().getMobile().get69()) {
            TextView tvNotificationSetting = (TextView) viewNotificationSetting.findViewById(R.id.notification_setting_title);
            tvNotificationSetting.setText(JsonPhp.getInstance().getLang().getMobile().get96());
        }
        ...
        if(JsonPhp.getInstance().getLang().getMobile().get88() != null){
            TextView shareAppTextView = (TextView) viewShareApp.findViewById(R.id.setting_share_title);
            shareAppTextView.setText(JsonPhp.getInstance().getLang().getMobile().get88());
        }

        if (null != config && null != config.getShareThisApp() && config.getShareThisApp().equals("0")) {
        ...
        if(JsonPhp.getInstance().getLang().getMobile().get109() != null){
            TextView inviteContactTextView = (TextView) viewInviteContact.findViewById(R.id.textInvitePhoneContact);
            inviteContactTextView.setText(JsonPhp.getInstance().getLang().getMobile().get109());
        }

        if (null != config && null != config.getInviteViaSms() && config.getInviteViaSms().equals("0")) {
            ...
            switch (popupId) {
                case LOGOUT:
                    ...
                    if (PreferenceHelper.contains(PreferenceKeys.LoginKeys.LOGGED_IN_AS_GUEST)) {
                        Intent intent = new Intent(SettingActivity.this, GuestLoginActivity.class);
                    ...
                    if (PreferenceHelper.contains(PreferenceKeys.LoginKeys.LOGGED_IN_AS_COD)) {
                        Intent intent = new Intent(SettingActivity.this, CoDLoginActivity.class);
                        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
                        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                        startActivity(intent);
