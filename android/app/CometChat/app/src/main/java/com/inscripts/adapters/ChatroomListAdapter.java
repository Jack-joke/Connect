...
public class ChatroomListAdapter extends ArrayAdapter<Chatroom> implements OnAlertDialogButtonClickListener,StickyListHeadersAdapter {
    ...
    private static final java.lang.String TAG = ChatroomListAdapter.class.getSimpleName();
    ...
    public ChatroomListAdapter(Context context, List<Chatroom> objects) {
        super(context, R.layout.custom_list_item_chatrooms, objects);
    ...
    public View getHeaderView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            holder = new ChatroomListAdapter.HeaderViewHolder();
            convertView = LayoutInflater.from(getContext()).inflate(R.layout.chatroom_list_header, parent, false);
            holder.chatroomStatus = (TextView) convertView.findViewById(R.id.message_date);
            convertView.setTag(holder);
        } else {
            holder = (ChatroomListAdapter.HeaderViewHolder) convertView.getTag();
        }
        Chatroom chatroom = getItem(position);

        if(chatroom.status == 1)
            if (language.getChatrooms().get78() != null) {
                holder.chatroomStatus.setText(language.getChatrooms().get78());
            } else {
                holder.chatroomStatus.setText("JOINED GROUPS");
            }
        else
        if (language.getChatrooms().get77() != null) {
            holder.chatroomStatus.setText(language.getChatrooms().get77());
        } else {
    ...
    public View getView(int position, View convertView, final ViewGroup parent) {
        ...
        if (null == view) {
            view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_chatroom_list, parent, false);
       ...
       // holder.menuBtnLeft.setBackgroundColor(Color.parseColor(PreferenceHelper.get(PreferenceKeys.Colors.COLOR_PRIMARY)));
        ...
        if(chatroom.status != 1){
            holder.menuBtnRename.setVisibility(View.GONE);
            holder.menuBtnRight.setVisibility(View.INVISIBLE);
            holder.sml.setSwipeEnable(false);
        }else{
            holder.menuBtnRight.setVisibility(View.VISIBLE);
            holder.sml.setSwipeEnable(true);
        }
        ...
        view.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                try {
                    Logger.error(TAG,"List on chatroomList item");
                            ...
                            if (createdBy == 1 || createdBy == 2) {
                                ...
                                switch (chatroom.type) {
                                    ...
                                    case CometChatKeys.ChatroomKeys.TypeKeys.PASSWORD_PROTECTED:
                                        View dialogview = ((Activity) parent.getContext()).getLayoutInflater().inflate(R.layout.custom_dialog, null);
                                        ...
                                        new CustomAlertDialogHelper(parent.getContext(), "", dialogview, language.getChatrooms().get19(),
                                                "", language.getChatrooms().get51(),ChatroomListAdapter.this, 1);
                                        break;
