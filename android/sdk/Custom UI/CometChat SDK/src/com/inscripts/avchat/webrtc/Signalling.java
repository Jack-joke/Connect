...
public class Signalling {
    ...
    public void start(final SingleAction<String> callback,String userId,boolean isBroadcast,boolean isInitiator) throws Exception {
        // Create a WebSync client.
        websyncClient = new Client(websyncServerUrl);
        
        if (isBroadcast) {
        	String hasVideo = isInitiator ? "false" : "true";
            Record record = new Record("hasVideo", hasVideo);
            BindArgs bindargs = new BindArgs(record);
            websyncClient.bind(bindargs);
        }
        
        Record record = new Record("userId", userId);
        BindArgs bindargs = new BindArgs(record);
        websyncClient.bind(bindargs);
