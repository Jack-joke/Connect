...
@SuppressLint("NewApi")
public class HardwareEncoder implements avp8.IEncoder
{
    ...
    public static MediaCodecInfo getCodecInfo(ArrayList<String> codecBlacklist)
    {
        ...
        MediaCodecInfo[] encoderInfos = avp8.HardwareUtility.getEncoderInfos(mimeType);
        ...
        encoderInfos = avp8.HardwareUtility.filterCodecInfos(encoderInfos, mimeType, colorFormats);
        if (encoderInfos.length == 0)
        {
    ...
    {	
    	try
    	{
	    	if (fourcc == nv21FourccFormat)
	    	{
		    	...
		    	// UV
		        Binary.deinterleaveTransform(frame, rotateFrame, (width/2), (height/2), (stride/2), rotation, pixelCount, true);
