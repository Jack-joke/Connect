...
public class SampleCometChatActivity extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final SubscribeCallbacks subCallbacks = new SubscribeCallbacks() {
			...
			public void onMessageReceived(JSONObject receivedMessage) {
                ...
                Log.e("abc","One-On-One onMessageReceived = "+receivedMessage);
				try {
			...
			public void gotProfileInfo(JSONObject profileInfo) {
				...
				Logger.debug("profile infor " + profileInfo);
				...
				try {
					...
					if (j.has("push_channel")) {
                        ...
                        FirebaseMessaging.getInstance().subscribeToTopic(j.getString("push_channel"));
					}
            ...
            public void gotBotList(JSONObject botList) {
                try {
                    LogsActivity.addToLog("One-On-One gotBotList");
					/* Store the list for later use. */
                    Logger.error("Bot List Json = "+botList);
                    SharedPreferenceHelper.save(SharedPreferenceKeys.BOT_LIST, botList.toString());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
			...
			public void onAVChatMessageReceived(JSONObject response) {
				try {
					...
					if (pluginType == 2) {
						...
						case 32:
							try {
								Intent i = new Intent(getApplicationContext(), SampleIncomingCallActivity.class);
						...
						case 36:
							...
							SampleIncomingCallActivity.incomingCallActivity.finish();
							break;
						case 37:
	...
	public void buttonClick(View view) {
		switch (view.getId()) {
            ...
            case R.id.buttonOpenBotList :
                startActivity(new Intent(this,BotListActivity.class));
                break;
